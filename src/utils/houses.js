...
export const calculateHousesWithSystem = async (ascendant, houseSystem = 'equal', latitude = 0, longitude = 0, date = new Date()) => {
  ...
  try {
    const swe = await import('sweph-wasm');
    if (swe) {
      // Best-effort attempt: APIs differ by package; detect common functions
      try {
        if (typeof swe.init === 'function') await swe.init();
      } catch (e) {}

      // If the module provides `swe_houses` (C-style API), call it
      ...
      // Some WASM builds expose a helper like `getHouses`.
  ...
  try {
    ...
    // `astronomia` may expose house algorithms under `house` (best-effort)
